<script setup>
import {reactive, computed, compile} from "vue";

// data 객체를 reactive함수를 통해 반응성 주입하여 초기화
const data = reactive({
  newItem:'',
  items:[],
});
...
// 완료 갯수
const isComplete = computed(()=>data.items.filter((item)=>item.completed === true).length);
...
<template>
  <main class="app">
    ...
    <div class="todo_add">
      <input type="text" placeholder="할 일을 입력하세요" title="할 일을 일력하세요" v-model="data.newItem" v-on:keyup.enter="addItem()" />
      <button type="button" class="add_btn" v-on:click="addItem()">Add</button>
    </div>
    <ul class="todo_list">
      <li v-for="(item) in data.items" v-bind:key="item.id" v-bind:class="{completed:item.completed}">
        <input type="checkbox" v-bind:id="`check${item.id}`" v-model="item.completed" />
